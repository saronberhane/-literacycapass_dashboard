  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if(selectedGenresList.length !==0)
        data.genre = selectedGenresList;
     
     if(selectedAuthorsList.length !==0)
        data.author = selectedAuthorsList;
  ...
  const fetchBook = async () => {
    ...
    try{
        ...
        if (response?.data?.status === "SUCCESS") {
            ...
            setDate(response.data?.data?.book.date);
            setLanguage(response.data?.data?.book.language);
            setNoOfPage(response.data?.data?.book.no_of_page);
            setAbout(response.data?.data?.book.about);
            setBookImage({cover_url: response.data?.data?.book.cover_url, 
            cover_pub_id: response.data?.data?.book.cover_pub_id})
        }
